#include "reactingGlobCompressibleFlow.hpp"
...
#include <flow/processes/tChemReactionsGlob.hpp>
#include <utilities/mpiError.hpp>

ablate::flow::ReactingGlobCompressibleFlow::ReactingGlobCompressibleFlow(std::string name, std::shared_ptr<mesh::Mesh> mesh, std::shared_ptr<eos::EOS> eosIn,
    ...
    : FVFlow(name, mesh, parameters,
              ...
              {.solutionField = false, .fieldName = "yi", .fieldPrefix = "yi", .components = (PetscInt)eosIn->GetSpecies().size(), .fieldType = FieldType::FV, .componentNames = eosIn->GetSpecies()}},
             {
                 ...
                 std::make_shared<ablate::flow::processes::TChemReactionsGlob>(std::dynamic_pointer_cast<eos::TChem>(eosIn) ? std::dynamic_pointer_cast<eos::TChem>(eosIn)
                                                                                                                        : throw std::invalid_argument("The eos must of type eos::TChem")),
             },
             options, initialization, boundaryConditions, {}, exactSolutions) {}

#include "parser/registrar.hpp"
REGISTER(ablate::flow::Flow, ablate::flow::ReactingGlobCompressibleFlow, "reacting compressible finite volume flow", ARG(std::string, "name", "the name of the flow field"),
