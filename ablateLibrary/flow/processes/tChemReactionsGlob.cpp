#include "tChemReactionsGlob.hpp"
...
ablate::flow::processes::TChemReactionsGlob::TChemReactionsGlob(std::shared_ptr<eos::EOS> eosIn, std::shared_ptr<parameters::Parameters> options)
    : fieldDm(nullptr),
      sourceVec(nullptr),
...
ablate::flow::processes::TChemReactionsGlob::~TChemReactionsGlob() {
    if (fieldDm) {
...
void ablate::flow::processes::TChemReactionsGlob::Initialize(ablate::flow::FVFlow& flow) {
    ...
    auto chemistryPreStage = std::bind(&ablate::flow::processes::TChemReactionsGlob::ChemistryFlowPreStage, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
...
PetscErrorCode ablate::flow::processes::TChemReactionsGlob::SinglePointChemistryRHS(TS ts, PetscReal t, Vec X, Vec F, void* ptr) {
    ablate::flow::processes::TChemReactionsGlob* solver = (ablate::flow::processes::TChemReactionsGlob*)ptr;
    PetscErrorCode ierr;
    PetscScalar* fArray;
    ...
    for(std::size_t i =0; i < solver->numberSpecies; i++){
        solver->tchemScratch[i+1] = PetscMin(PetscMax(solver->tchemScratch[i+1], 1E-10), 1);
    }
...
PetscErrorCode ablate::flow::processes::TChemReactionsGlob::SinglePointChemistryJacobian(TS ts, PetscReal t, Vec X, Mat aMat, Mat pMat, void* ptr) {
    ablate::flow::processes::TChemReactionsGlob* solver = (ablate::flow::processes::TChemReactionsGlob*)ptr;
    PetscErrorCode ierr;
    ...
    PetscInt spLoc = 5+1;
    PetscInt loc = (solver->numberSpecies + 1)*spLoc + spLoc;
...
PetscErrorCode ablate::flow::processes::TChemReactionsGlob::ChemistryFlowPreStage(TS flowTs, ablate::flow::Flow& flow, PetscReal stagetime) {
    PetscInt stepNumber;
    ...
    for (PetscInt c = cStart; c < cEnd; ++c) {
        ...
        if (euler) {
            ...
            for (std::size_t s = 0; s < numberSpecies; s++) {
                pointArray[s + 1] = PetscMin(PetscMax(1.0E-20, densityYi[s] / euler[ablate::flow::processes::EulerAdvection::RHO]), 1.0);
            }
...
PetscErrorCode ablate::flow::processes::TChemReactionsGlob::AddChemistrySourceToFlow(DM dm, PetscReal time, Vec locX, Vec fVec, void* ctx) {
    IS cellIS;
    DM plex;
    PetscInt depth;
    PetscErrorCode ierr;
    ...
    ablate::flow::processes::TChemReactionsGlob* solver = (ablate::flow::processes::TChemReactionsGlob*)ctx;
...
REGISTER(ablate::flow::processes::FlowProcess, ablate::flow::processes::TChemReactionsGlob, "reactions using the TChem v1 library", ARG(eos::EOS, "eos", "the tChem v1 eos"),
