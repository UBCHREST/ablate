...
ablate::domain::modifier::CreateLabel::CreateLabel(std::string name, std::shared_ptr<mathFunctions::MathFunction> function, int dmDepth, int labelValueIn)
    : name(name),
      function(function),
      dmDepth((PetscInt)dmDepth),
      labelValue(labelValueIn == 0 ? 1 : (PetscInt)labelValueIn)
{}
...
REGISTER(ablate::domain::modifier::Modifier, ablate::domain::modifier::CreateLabel, "Creates a new label for all positive points in the function",
         ARG(std::string, "name", "the new label name"),
         ARG(mathFunctions::MathFunction, "function", "the function to evaluate"),
