...
void ablate::chemistry::ChemTabModel::ChemTabModelComputeSourceFunction(const PetscReal progressVariables[], const std::size_t progressVariablesSize, PetscReal ZMix, PetscReal *predictedSourceEnergy, PetscReal* progressVariableSource, const std::size_t progressVariableSourceSize, void* ctx){
	auto ctModel = (ChemTabModel *)ctx;
	//size of progressVariables should match the expected number of progressVariables
	if(progressVariablesSize != ctModel->progressVariablesNames.size()){
		throw std::invalid_argument("The progressVariables size does not match the supported number of progressVariables");
	...
	if(t0.oper == NULL)
		throw std::runtime_error("ERROR: Failed TF_GraphOperationByName serving_default_input_1");
	input[0] = t0;
	//********* Get Output tensor
	int numOutputs = 1;
	...
	int ndims = 2;
	int ninputs = 1 + (int) ctModel->progressVariablesNames.size();
	int64_t dims[] = {1,ninputs};
	float data[ctModel->progressVariablesNames.size()+1];
	data[0] = ZMix;
	for(int i = 0; i < ninputs ;i++){
		data[i+1] = progressVariables[i];
	}
...
const std::vector<std::string> &ablate::chemistry::ChemTabModel::GetProgressVariables() const { 
