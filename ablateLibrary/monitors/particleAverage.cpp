...
void ablate::monitors::ParticleAverage::Register(std::shared_ptr<Monitorable> monitorableObject) {
...
PetscErrorCode ablate::monitors::ParticleAverage::OutputParticleAverage(TS ts, PetscInt steps, PetscReal time, Vec u, void *mctx) {
    ...
    if (steps == 0 || monitor->interval == 0 || (steps % monitor->interval == 0)) {
        ...
        ierr = DMSwarmGetLocalSize(monitor->particles->GetDM(), &localParticleCount);
        CHKERRQ(ierr);
        ierr = DMSwarmGetSize(monitor->particles->GetDM(), &globalParticleCount);
        CHKERRQ(ierr);
        ...
        ierr = DMSwarmGetField(monitor->particles->GetDM(), DMSwarmPICField_coor, &dims, NULL, (void **)&coords);
        CHKERRQ(ierr);
        ...
        ierr = DMSwarmRestoreField(monitor->particles->GetDM(), DMSwarmPICField_coor, &dims, NULL, (void **)&coords);
        CHKERRQ(ierr);
        ...
        int mpiErr = MPI_Reduce(avg, globAvg, bufferSize, MPIU_REAL, MPI_SUM, 0, PetscObjectComm((PetscObject)monitor->particles->GetDM()));
        CHKERRMPI(mpiErr);

        // print to the log
        monitor->log->Printf("%s ", monitor->particles->GetName().c_str());
