...
$$\begin{equation}\frac{\partial (\bar{\rho} \tilde{u}_{i})}{\partial t} + \frac{\partial(\bar{\rho} \tilde{u}_{i}\tilde{u}_{j})}{\partial x_{j}} = 
-\frac{\partial \bar{p}}{\partial x_{i}} + \frac{\partial \mu \tilde{S}_{ij}}{\partial x_{j}} - \frac{\partial \tau_{ij}}{\partial x_{j}}\end{equation}$$
...
Where $\tau_{ij}=\bar{\rho} \widetilde{u_{i}u_{j}}-\bar{\rho} \tilde{u}_{i}\tilde{u}_{j}$ is the subgrid-scale (SGS) stress tensor, $q_{j}=\bar{\rho} C_p \widetilde{u_{j}T}-\bar{\rho} C_p \tilde{u}_{j}\tilde{T}$, the SGS heat flux, representing the effect of the small scales of turbulence on the large ones, and $J_j$ is the species transport due to the SGS
velocity fluctuations,. Here ($\;\bar{}\;$) and ($\;\tilde{}\;$) imply the averaged value and the density-weighted averaged (Favre-averaged) value, respectively. The set of equations is closed by setting $\tau_{ij} = -2 \mu_t \widetilde{S_{ij}}$, $q_j =-C_p \frac{\mu_t}{Pr_t} \frac{\partial \tilde{ T}}{\partial x_j}$, and $J_j =- \frac{\mu_t}{Sc_t} \frac{\partial \tilde{ Y_i}}{\partial x_j}$
 where $\widetilde{S_{ij}}\equiv \frac{1}{2} \left(\frac{\partial \widetilde{ u_i}}{\partial x_j} +
                \frac{\partial \widetilde{u_j}}{\partial x_i} \right) -
                \frac{1}{3} \frac{\partial \widetilde{u_k}}{\partial x_k} \delta_{ij}$ and $\mu_t$ is the turbulent viscosity, and by the modified averaged Equation of State, $\bar{P}= \bar{\rho} R \tilde{T}$. $Pr_t$ and $Sc_t$ are assumed to be $1$ based on Reynolds analogy.
...
$$\begin{equation}\frac{\partial (\bar{\rho }k)}{\partial t} + \frac{\partial (\bar{\rho } \tilde{u}_j k)}{\partial x_{j}} -\frac{\partial}{\partial x_{j}} \left[ \bar{\rho }\\
\left(\nu + \nu_t\right) \frac{\partial k}{\partial x_{j}} \right] =-\bar{\rho } \tau_{ij} .\widetilde{ S_{ij}} \; -\; C_{\epsilon} \frac{\bar{\rho } k^{3/2}}{\Delta},\end{equation}$$
