...
struct SootProgressTransportModelTestParameters {
...
class SootProgressTransportModelTestFixture : public ::testing::TestWithParam<SootProgressTransportModelTestParameters> {};

TEST_P(SootProgressTransportModelTestFixture, ShouldComputePropertiesForFunction) {
    // ARRANGE
    auto baseTransportModel = GetParam().getBaseTransportModel();

    // Create a soot transport property
    auto sootSpeciesTransportModel = std::make_shared<ablate::eos::tChemSoot::SootProgressTransportModel>(baseTransportModel);
...
TEST_P(SootProgressTransportModelTestFixture, ShouldComputePropertiesForTemperatureFunction) {
    // ARRANGE
    auto baseTransportModel = GetParam().getBaseTransportModel();
...
INSTANTIATE_TEST_SUITE_P(SootSpeciesTransportTests, SootProgressTransportModelTestFixture,
                         testing::Values(
                         ...
                         [](const testing::TestParamInfo<SootProgressTransportModelTestParameters>& info) { return std::to_string(info.index) + "_" + info.param.name; });
