...
static struct CompressibleSpeciesDiffusionTestParameters {
...
static class CompressibleFlowSpeciesDiffusionTestFixture : public testingResources::MpiTestFixture, public ::testing::WithParamInterface<CompressibleSpeciesDiffusionTestParameters> {
   public:
...
const static std::vector<std::string> progress = {"pg0", "pg1"};
...
TEST_P(CompressibleFlowSpeciesDiffusionTestFixture, ShouldConvergeToExactSolution) {
    StartWithMPI
        ...
        for (PetscInt l = 1; l <= GetParam().levels; l++) {
            ...
            EXPECT_CALL(*eos, GetProgressVariables()).Times(::testing::AtLeast(1)).WillRepeatedly(::testing::ReturnRef(progress));
            ...
            std::vector<std::shared_ptr<ablate::domain::FieldDescriptor>> fieldDescriptors = {
                ...
                std::make_shared<ablate::domain::FieldDescription>(ablate::finiteVolume::CompressibleFlowFields::DENSITY_YI_FIELD, ablate::finiteVolume::CompressibleFlowFields::DENSITY_YI_FIELD, eos->GetSpeciesVariables(), domain::FieldLocation::SOL, domain::FieldType::FVM),
                std::make_shared<ablate::domain::FieldDescription>(ablate::finiteVolume::CompressibleFlowFields::YI_FIELD, ablate::finiteVolume::CompressibleFlowFields::YI_FIELD, eos->GetSpeciesVariables(), domain::FieldLocation::AUX, domain::FieldType::FVM),
                std::make_shared<ablate::domain::FieldDescription>(ablate::finiteVolume::CompressibleFlowFields::DENSITY_PROGRESS_FIELD, ablate::finiteVolume::CompressibleFlowFields::DENSITY_PROGRESS_FIELD, eos->GetProgressVariables(), domain::FieldLocation::SOL, domain::FieldType::FVM),
                std::make_shared<ablate::domain::FieldDescription>(ablate::finiteVolume::CompressibleFlowFields::PROGRESS_FIELD, ablate::finiteVolume::CompressibleFlowFields::PROGRESS_FIELD, eos->GetProgressVariables(), domain::FieldLocation::AUX, domain::FieldType::FVM),
            ...
            auto flowProcesses = std::vector<std::shared_ptr<ablate::finiteVolume::processes::Process>>{
                std::make_shared<ablate::finiteVolume::processes::SpeciesTransport>(eos),
            };
            ...
            auto auxVec = flowObject->GetSubDomain().GetAuxVector();
            auto temperatureFieldFunction = std::make_shared<ablate::mathFunctions::FieldFunction>(ablate::finiteVolume::CompressibleFlowFields::TEMPERATURE_FIELD, std::make_shared<ablate::mathFunctions::Linear>(std::vector<double>{0.0}, std::vector<double>{GetParam().parameters.L}, 1000, 300, 1));
            flowObject->GetSubDomain().ProjectFieldFunctionsToLocalVector({temperatureFieldFunction}, auxVec);
