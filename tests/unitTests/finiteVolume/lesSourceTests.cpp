...
static PetscReal ComputeTExact(PetscReal time, const PetscReal xyz[], InputParameters *parameters) {
    ...
    PetscReal alphaT = 1.040 *  0.094 * sqrt(parameters->ev) * parameters->L / ( cv);
    ...
    for (PetscReal n = 1; n < 2000; n++) {
        ...
        T += Bn * PetscSinReal(n * PETSC_PI * xyz[0] / parameters->L) * PetscExpReal(-n * n * PETSC_PI * PETSC_PI * (alpha + alphaT) * time / (PetscSqr(parameters->L)));
    }
...
TEST_P(CompressibleFlowDiffusionTestFixture, ShouldConvergeToExactSolution) {
    StartWithMPI
        ...
        for (PetscInt l = 0; l < GetParam().levels; l++) {
            ...
            std::vector<std::shared_ptr<ablate::domain::FieldDescriptor>> fieldDescriptors = {std::make_shared<ablate::finiteVolume::CompressibleFlowFields>(eos, std::vector<std::string>{"ev1"})};
            ...
            auto additionalProcesses = std::vector<std::shared_ptr<ablate::finiteVolume::processes::Process>>{std::make_shared<finiteVolume::processes::LES>("parameters.ev")};
...
INSTANTIATE_TEST_SUITE_P(CompressibleFlow, CompressibleFlowDiffusionTestFixture,
                         testing::Values((CompressibleFlowDiffusionTestParameters){.mpiTestParameter = {.testName = "conduction",
                                                                                   ...
                                                                                   .parameters = {.dim = 2, .L = 0.1, .gamma = 1.4, .Rgas = 1.0, .k = 0.3, .tke = 3, .rho = 1.0, .Tinit = 400, .Tboundary = 300},
                                                                                   .initialNx = 3,
                                                                                   .levels = 3,
