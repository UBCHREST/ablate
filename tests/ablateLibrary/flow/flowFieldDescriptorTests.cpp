...
TEST(FieldDescriptors, ShouldBeCreatedByFactoryFunction) {
    ...
    EXPECT_CALL(*mockSubFactory, Contains(std::string("prefix"))).Times(::testing::Exactly(1)).WillOnce(::testing::Return(true));
...
TEST(FieldDescriptors, ShouldBeCreatedByFactoryFunctionWithMinimalInputs) {
    ...
    // assert
    ASSERT_EQ(flowField->type, ablate::domain::FieldType::SOL);
    ASSERT_EQ(flowField->name, "name1");
