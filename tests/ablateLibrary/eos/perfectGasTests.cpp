...
INSTANTIATE_TEST_SUITE_P(EOSTests, PerfectGasTestCreateAndViewFixture,
                         testing::Values((EOSTestCreateAndViewParameters){.options = {}, .expectedView = "EOS: perfectGas\n\tgamma: 1.4\n\tRgas: 287\n"},
                                         (EOSTestCreateAndViewParameters){
                                             .options = {{"gamma", "3.2"}, {"Rgas", "100.2"}}, .expectedView = "EOS: perfectGas\n\tgamma: 3.2\n\tRgas: 100.2\n"}),
...
TEST_P(PerfectGasTestDecodeStateFixture, ShouldDecodeState) {
    ...
    // act
    PetscErrorCode ierr = eos->GetDecodeStateFunction()(&params.yiIn[0], params.velocityIn.size(), params.densityIn, params.totalEnergyIn, &params.velocityIn[0], &internalEnergy, &speedOfSound, &pressure, eos->GetDecodeStateContext());
...
INSTANTIATE_TEST_SUITE_P(EOSTests, PerfectGasTestDecodeStateFixture,
                         testing::Values((EOSTestDecodeStateParameters){
...
TEST_P(PerfectGasTestTemperatureFixture, ShouldComputeTemperature) {
    ...
    // act
    PetscErrorCode ierr = eos->GetComputeTemperatureFunction()(&params.yiIn[0], params.massFluxIn.size(), params.densityIn, params.totalEnergyIn, &params.massFluxIn[0], &temperature, eos->GetComputeTemperatureContext());
...
INSTANTIATE_TEST_SUITE_P(
    ...
    testing::Values(
        ...
        (EOSTestTemperatureParameters){ .options = {{"gamma", "2.0"}, {"Rgas", "4.0"}}, .yiIn = {}, .densityIn = .9, .totalEnergyIn = 1.56E5, .massFluxIn = {0.0}, .expectedTemperature = 39000}),
