...
void ablate::monitors::PhysicsTimeStep::Register(std::shared_ptr<solver::Solver> solver) {
    Monitor::Register(solver);

    fvmSolver = std::dynamic_pointer_cast<ablate::finiteVolume::FiniteVolumeSolver>(solver);
    if (!fvmSolver) {
    ...
    if (!log->Initialized()) {
        log->Initialize(solver->GetSubDomain().GetComm());
    }
}
...
PetscErrorCode ablate::monitors::PhysicsTimeStep::ReportPhysicsTimeStep(TS ts, PetscInt step, PetscReal crtime, Vec, void* ctx) {
    ...
    if (monitor->interval->Check(PetscObjectComm((PetscObject)ts), step, crtime)) {
        auto dts = monitor->fvmSolver->ComputePhysicsTimeSteps(ts);
