#include "lesCompressibleFlowSolver.hpp"
#include <utility>
...
ablate::finiteVolume::LesCompressibleFlowSolver::LesCompressibleFlowSolver(std::string solverId, std::shared_ptr<domain::Region> region, std::shared_ptr<parameters::Parameters> options,
                         ...
                         utilities::VectorUtilities::Merge(
                             {
                                 ...
                                 std::make_shared<ablate::finiteVolume::processes::LES>(eosIn, transport),
...
ablate::finiteVolume::LesCompressibleFlowSolver::LesCompressibleFlowSolver(std::string solverId, std::shared_ptr<domain::Region> region, std::shared_ptr<parameters::Parameters> options,
                                                                           ...
                                                                           const std::shared_ptr<fluxCalculator::FluxCalculator>& fluxCalculatorIn,
                                                                           std::vector<std::shared_ptr<boundaryConditions::BoundaryCondition>> boundaryConditions, bool computePhysicsTimeStep)
    : LesCompressibleFlowSolver(std::move(solverId), std::move(region), std::move(options), eosIn, parameters, transport, fluxCalculatorIn, {}, std::move(boundaryConditions), computePhysicsTimeStep) {
}

#include "registrar.hpp"
REGISTER(ablate::solver::Solver, ablate::finiteVolume::LesCompressibleFlowSolver, "les compressible finite volume flow", ARG(std::string, "id", "the name of the flow field"),
