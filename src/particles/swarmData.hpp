#ifndef ABLATELIBRARY_SWARMDATA_HPP
#define ABLATELIBRARY_SWARMDATA_HPP
...
class SwarmData {
   ...
   public:
    SwarmData(const DM& swarmDm, const std::map<std::string_view, Field>& fieldsMap, Vec solutionVec) : swarmDm(swarmDm), fieldsMap(fieldsMap), solutionVec(solutionVec) {
        // extract the array from the vector
        VecGetArray(solutionVec, &solutionValues) >> checkError;
    }

    ~SwarmData() { VecRestoreArray(solutionVec, &solutionValues) >> checkError; }
