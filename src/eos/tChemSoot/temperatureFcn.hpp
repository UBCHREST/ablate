...
#include "eos/tChemSoot/totalInternalEnergyFcn.hpp"
...
struct TemperatureFcn {
    ...
    template <typename MemberType, typename KineticModelConstDataType>
    KOKKOS_INLINE_FUNCTION static value_type team_invoke_fromTotEnergy(const MemberType& member,
                                                         /// input
                                                         const Impl::StateVector<Tines::value_type_1d_view<real_type, DeviceType>> svGas, // Gaseous State Vector
                                                         const real_type Yc,
                                                         const Tines::value_type_0d_view<real_type, DeviceType> totalInternalEnergyRef,
                                                         const real_type_1d_view_type hi_Scratch,
                                                         const real_type_1d_view_type cp_gas_Scratch,
                                                         const KineticModelConstDataType& kmcd) {
        TCHEM_CHECK_ERROR(!svGas.isValid(), "Error: input state vector is not valid");
        {
            ...
            // set some constants
            const auto EPS_T_RHO_E = 50;
            const auto ITERMAX_T = 100;

            // compute the first error
            double e2 = ablate::eos::tChemSoot::impl::TotalInternalEnergyFcn<real_type, DeviceType>::team_invoke(member, Yc, svGas, hi_Scratch, cp_gas_Scratch, kmcd);
            double f2 = totalInternalEnergyRef() - e2;
            if (Tines::ats<real_type>::abs(f2) > EPS_T_RHO_E) {
                double t0 = t2;
                double f0 = f2;
                double t1 = t0 + 1;
                ...
                t = t1;
                double e1 = ablate::eos::tChemSoot::impl::TotalInternalEnergyFcn<real_type, DeviceType>::team_invoke(member, Yc, svGas, hi_Scratch, cp_gas_Scratch, kmcd);
                double f1 = totalInternalEnergyRef() - e1;

                for (int it = 0; it < ITERMAX_T; it++) {
                    t2 = t1 - f1 * (t1 - t0) / (f1 - f0 + 1E-30);
                    t2 = std::max(1.0, t2);
                    t = t2;
                    e2 = ablate::eos::tChemSoot::impl::TotalInternalEnergyFcn<real_type, DeviceType>::team_invoke(member, Yc, svGas, hi_Scratch, cp_gas_Scratch, kmcd);
                    f2 = totalInternalEnergyRef() - e2;
                    if (Tines::ats<real_type>::abs(f2) <= EPS_T_RHO_E) {
                        t = t2;
                        // Make Sure Temperature is set in the stateSpace
                        return svGas.Temperature();
