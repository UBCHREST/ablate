...
inline double NdNuclationConversionTerm =
    2. / Cmin * 6.02214076e26;  // this is the soot conversion term used to convert the nucleation rate for carbon to its rate for soot number density = 2 (avogadro's number)/Cmin
...
inline double calculateNucleationReactionRate(double T, double C2H2Conc, double fv) { return 1000. * std::exp(-16103. / T) * C2H2Conc * (1 - fv); }

inline double calculateSurfaceGrowthReactionRate(double T, double C2H2Conc, double SA_V) { return 700. * std::exp(-10064. / T) * C2H2Conc * std::sqrt(SA_V); }

inline double calculateAgglomerationRate(double YCarbon, double Nd, double T, double totalDensity) {
    double dp = calculateSootDiameter(YCarbon, Nd);
    return 2 * Ca * std::sqrt(dp) * std::sqrt(6 * (1.380649e-23) * T / tChemSoot::solidCarbonDensity) * (totalDensity * totalDensity * Nd * Nd);  // the Term in the paranthesis is Boltzman's constant
}
...
inline double calculateOOxidationRate(double OConc, double T, double SA_V, double fv) { return 0.001094 * OxidationCollisionEfficiency * std::sqrt(T) * (8314.4626) * OConc * SA_V * (1 - fv); }

inline double calculateOHOxidationRate(double OHConc, double T, double SA_V, double fv) { return .001044 * OxidationCollisionEfficiency * std::sqrt(T) * (8314.4626) * OHConc * SA_V * (1 - fv); }
...
static void UpdateSpeciesSpecificIndices(std::vector<std::string> species) {
    ...
    // Note that since the species vector will always start with Y_C(s) and the dependent variable will always start with temperature, the gas phase species will share the same index in the dependent
    // variable
...
template <typename device_type>
KOKKOS_INLINE_FUNCTION static void UpdateSourceWithSootMechanismRates(const Tines::value_type_1d_view<real_type, device_type>& x, const Tines::value_type_1d_view<real_type, device_type>& f,
                                                                      const real_type& densityTotal, KineticModelConstData<device_type> _kmcd) {
...
KOKKOS_INLINE_FUNCTION static void UpdateSourceWithSootMechanismRatesTemperature(MemberType& member, const Tines::value_type_1d_view<real_type, device_type>& f, const real_type& YCarbon,
                                                                                 const real_type& Nd, ::TChem::Impl::StateVector<Tines::value_type_1d_view<real_type, device_type>> gas_SV,
                                                                                 const real_type& densityTotal, const Tines::value_type_1d_view<real_type, device_type>& cp_Scratch,
    ...
    C2H2Conc = rho_g * PetscMax(0, Yi_g[C2H2Ind - 1]) /
               _kmcd.sMass(C2H2Ind - 1);  // Note that the indices save are for the x vector and are actually 1 more than their corresponding spot in the gas mixture array
    ...
    Kokkos::parallel_for(Kokkos::TeamVectorRange(member, _kmcd.nSpec), [&](const ordinal_type& i) { cp_Scratch(i) = 1 / _kmcd.sMass(i) * (hi_gas(i) - _kmcd.Runiv * Temperature); });
    member.team_barrier();

    // The temperature equations is a behemoth,
    // First Grab Some carbon Properties at this temperature
    double HC_R_MWC = Soot::CarbonEnthalpy_R_T(Temperature) * Temperature / tChemSoot::MWCarbon;
    real_type denom = Cv_gas * (1 - YCarbon) + YCarbon * _kmcd.Runiv * (HC_R_MWC - rho_g / tChemSoot::solidCarbonDensity / MW_gas);
    real_type YCD = f(_kmcd.nSpec + 1);
    // Start the RHS with the Carbon component
    real_type RHS = (_kmcd.Runiv * YCarbon * Temperature / MW_gas / tChemSoot::solidCarbonDensity * YCD * (1 / densityTotal - 1 / tChemSoot::solidCarbonDensity) /
                     (1 / densityTotal - YCarbon / tChemSoot::solidCarbonDensity) / (1 / densityTotal - YCarbon / tChemSoot::solidCarbonDensity));
    //        std::cout << "YC: " << YCarbon << ", YCD: " << YCD << ", Rho_T: " << densityTotal;
    // Now Add in the summation terms
    real_type T1 = 0;
    real_type T2 = 0;
    Kokkos::parallel_for(Kokkos::TeamVectorRange(member, _kmcd.nSpec), [&](const ordinal_type& i) {
    ...
    // Scale T2 Correctly
    T2 *= YCarbon * _kmcd.Runiv * rho_g * Temperature / tChemSoot::solidCarbonDensity;

    // Add Correct Soot Term To T1
    double sootIntEnergy = HC_R_MWC * _kmcd.Runiv - gas_SV.Pressure() / tChemSoot::solidCarbonDensity;
