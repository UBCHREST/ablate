...
void ablate::domain::modifiers::CompleteLabel::Modify(DM &dm) {
    ...
    DMGetLabel(dm, region->GetName().c_str(), &completeLabel) >> utilities::PetscUtilities::checkError;

    if (completeLabel) {
        DMPlexLabelComplete(dm, completeLabel) >> utilities::PetscUtilities::checkError;
    }
}
