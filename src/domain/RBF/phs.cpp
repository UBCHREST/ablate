...
PHS::PHS(PetscInt p, PetscInt phsOrder, bool hasDerivatives, bool hasInterpolation) :
  ...
  phsOrder(phsOrder < 1 ? __RBF_PHS_DEFAULT_PARAM : 2*phsOrder+1) {};
...
PetscReal PHS::RBFDer(PetscReal xIn[], PetscInt dx, PetscInt dy, PetscInt dz) {
  const PetscInt m = PHS::phsOrder;
  const PetscReal r2 = PHS::DistanceSquared(xIn);
